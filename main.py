...
def color_states_recursive(states_dict, colors, colored_dict, state_list, index):
    ...
    for color in colors:
        if is_safe(state, color, colored_dict, states_dict):
            ...
            if color_states_recursive(states_dict, colors, colored_dict, state_list, index + 1):
                return True
...
def color_states(states_dict, colors):
    ...
    if color_states_recursive(states_dict, colors, colored_dict, state_list, 0): return colored_dict
...
fig.savefig('grovers_circuit.png')
plt.close(fig)
